...
/*  Created On : 09-4月-2018 19:30:21 				*/
/*  DBMS       : MySql 						*/
/* ---------------------------------------------------- */
...
DROP TABLE IF EXISTS `customer_virtual_product` CASCADE
;
...
CREATE TABLE `customer_virtual_product`
(
	`id` INTEGER NOT NULL AUTO_INCREMENT ,
	`user_id` INTEGER NOT NULL COMMENT '用户id',
	`garage_id` INTEGER NOT NULL COMMENT '车行id',
	`product_type` VARCHAR(50) NOT NULL COMMENT '产品类型，比如洗车卡，优惠券',
	`for_type` VARCHAR(50) NOT NULL COMMENT '可用类型，比如优惠券只可用于钣金，洗车卡只可用于洗车，全类型选择ALL',
	`for_brand` VARCHAR(50) NOT NULL COMMENT '可用品牌，比如优惠券可用于特定品牌产品，全品牌可选择ALL',
	`product_name` VARCHAR(50) NOT NULL COMMENT '产品用于显示的名称',
	`product_desc` VARCHAR(50) NOT NULL COMMENT '产品用于显示的描述',
	`start_date` DATE NOT NULL COMMENT '产品使用的开始日期',
	`end_date` DATE NOT NULL COMMENT '产品使用的到期日',
	`remain_count` INTEGER NOT NULL COMMENT '产品剩余可用次数，0即为不可用状态',
	`coupon_discount` FLOAT(0) NOT NULL COMMENT '折扣，无折扣产品填1',
	`coupon_price` FLOAT(0) NOT NULL COMMENT '可抵扣的价格，无价格可抵扣填0',
	`coupon_condition` FLOAT(0) NOT NULL COMMENT '使用前提条件，比如500可抵扣10元优惠券，500为前提条件，无前提条件产品填0',
	CONSTRAINT `PK_customer_virtual_product` PRIMARY KEY (`id`)
)
;
...
ALTER TABLE `customer_virtual_product` 
 ADD INDEX `IXFK_customer_virtual_product_sys_user` (`id` ASC)
;
