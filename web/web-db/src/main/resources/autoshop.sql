...
/*  Created On : 17-4月-2018 20:26:13 				*/
/*  DBMS       : MySql 						*/
/* ---------------------------------------------------- */
...
DROP TABLE IF EXISTS `customer_consume_maintain` CASCADE
;
...
CREATE TABLE `customer_consume_maintain`
(
	`id` INTEGER NOT NULL,
	`consume_id` INTEGER COMMENT '消费记录ID',
	`maintain_id` INTEGER COMMENT '服务记录ID',
	CONSTRAINT `PK_customer_consume_maintain` PRIMARY KEY (`id`)
) COMMENT='消费服务关联表'
;
...
ALTER TABLE `customer_consume_maintain` 
 ADD INDEX `IXFK_customer_consume_maintain_customer_consume_record` (`consume_id` ASC)
;

ALTER TABLE `customer_consume_maintain` 
 ADD INDEX `IXFK_customer_consume_maintain_vehicle_maintain_info` (`maintain_id` ASC)
;
